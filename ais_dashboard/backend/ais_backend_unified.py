...
ENABLE_VIDEO_PROCESSING = os.getenv("ENABLE_VIDEO_PROCESSING", "true").lower() in ["true", "1", "yes"]
VIDEO_PATH = os.getenv("VIDEO_PATH", "path/to/video.mp4")
...
# Current operating mode (can be switched at runtime)
current_mode = "hybrid" if ENABLE_VIDEO_PROCESSING else "ais-only"
    ...
    while True:
        try:
            ...
            if ENABLE_VIDEO_PROCESSING and current_mode == "hybrid" and video_frame_queue is not None:
                try:
                    frame_output: FrameOutput = video_frame_queue.get_nowait()
...
async def lifespan(app: FastAPI):
    ...
    if ENABLE_VIDEO_PROCESSING:
        try:
            ...
            print("   Continuing in AIS-only mode...")
            video_orchestrator = None
    else:
        print("   Starting in AIS-ONLY mode (no video processing)")
        video_orchestrator = None
    ...
    # Start video processing only if orchestrator is available
    if video_orchestrator and ENABLE_VIDEO_PROCESSING:
        asyncio.create_task(video_orchestrator.process_video(VIDEO_PATH))

    print("✅ All tasks started")
    if ENABLE_VIDEO_PROCESSING:
        print("   Ready: Live AIS + Video + LSTM")
    else:
        print("   Ready: Live AIS only")
...
async def set_mode(mode: str):
    ...
    if not ENABLE_VIDEO_PROCESSING and mode == "hybrid":
        return {"success": False, "error": "Hybrid mode not available. Video processing is disabled at startup."}
